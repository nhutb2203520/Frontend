<template>
    <div class="container-fluid bg-dark text-white py-5">
      ...
      <div class="position-relative bg-secondary bg-opacity-25 rounded shadow p-4 overflow-hidden">
        <div
          class="row g-4 flex-nowrap overflow-hidden"
        ...
        <button
          class="btn btn-info rounded-circle position-absolute top-50 start-0 translate-middle-y"
          @click="prevSlide"
        ...
        <button
          class="btn btn-info rounded-circle position-absolute top-50 end-0 translate-middle-y"
          @click="nextSlide"
      ...
      <div class="d-flex justify-content-center gap-3 mt-4">
        <span
          v-for="(dot, index) in Math.ceil(books.length / itemsPerView)"
          :key="index"
  ...
  export default {
    ...
    mounted() {
      ...
      // Lưu danh sách books vào localStorage để BookDetails lấy ra
      localStorage.setItem('hotBooks', JSON.stringify(this.books));
    },
    beforeUnmount() {
